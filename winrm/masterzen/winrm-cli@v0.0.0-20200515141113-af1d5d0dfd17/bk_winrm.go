...
func main() {
	...
	flag.StringVar(&command, "cmd", "", "Command to execute on remote Windows machine")

	flag.Parse()
	
	if command == "" {
    if flag.NArg() == 0 {
         check(errors.New("Please provide the command to execute using -cmd or as positional arguments"))
    }
    command = flag.Args()[0]
}
